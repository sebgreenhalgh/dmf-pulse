...
- P3 GBDT and P4 advanced latent/Bayesian challengers are deferred; no substitute class is faked.
- Exact path enumeration is bounded by strict configuration.
- Configured horizon opportunity counts and all recurrent/flow/ownership numerical values remain
  provisional synthetic policy parameters, not measured 2026/27 provider behavior.
- Live ACT/WAIT decisions fail closed to manual review until activation gates are satisfied.
- The single complete repository pytest run reached the 20-minute execution ceiling without a
  final summary or emitted failure trace. Its status is `RESOURCE_LIMIT`, not `PASS`.
