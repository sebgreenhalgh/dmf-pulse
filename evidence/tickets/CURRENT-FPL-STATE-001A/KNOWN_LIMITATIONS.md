...
- Windows Application Control blocks the generated `pytest` console launcher in this local
  ...
  `uv run python -m pytest` (57 focused and 273 inherited tests passed); GitHub Actions must prove
  the canonical launcher on the final Linux SHA. No failed assertion was hidden by this boundary.
- Official target-event flags are accepted only when the requested event is explicitly current or
  next, unique, and unfinished. Historical/future planning outside that published state is not
  supported.
