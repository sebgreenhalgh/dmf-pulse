...
Updated: 2026-07-25T13:44:32+01:00
...
- 2026-07-25T13:40:01+01:00 - the first clean literal run passed commands 1-23 and guaranteed PostgreSQL teardown, then failed closed before command 24 because the review privacy scanner's lowercase sentinel survived title-case-only redaction in the complete patch. The archive was not retained. Case-insensitive full-name, username, surname, and Windows-user-path redaction now also removes self-referential scanner literals; 17 focused review/evidence tests, Ruff, and strict mypy pass. The fix will be committed separately and all 25 commands rerun from the new clean HEAD.
- 2026-07-25T13:44:32+01:00 - the second clean literal run passed commands 1-10 and then proved command 11 was improperly coupled to the not-yet-started PostgreSQL service: six database-dependent tests inherited a module-wide `security` marker despite already carrying `postgres`. The three offline tests now carry `security` explicitly; literal command 11 passes with the service down (6 passed, 6 deselected, zero skips), while command 18 still executes the complete nine-test rights/raw-retention file after migration. Another clean full rerun is required.
