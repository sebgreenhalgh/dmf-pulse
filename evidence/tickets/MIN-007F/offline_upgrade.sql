...
CREATE TABLE football.player_minutes_projection (
    ...
    confidence_grade VARCHAR(1) NOT NULL,
    confidence_reasons VARCHAR(64)[] NOT NULL,
    created_at TIMESTAMP WITH TIME ZONE DEFAULT transaction_timestamp() NOT NULL,
    ...
    CONSTRAINT ck_player_minutes_projection_position CHECK (position IN ('GK','DEF','MID','FWD')),
    CONSTRAINT ck_player_minutes_projection_confidence_grade CHECK (confidence_grade IN ('A','B','C','D')),
    CONSTRAINT ck_player_minutes_projection_confidence_reasons CHECK (football.validate_minutes_confidence_reasons(confidence_reasons)),
