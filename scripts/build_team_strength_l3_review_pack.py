"""Explicit capped L3 Phase A source/evidence archive; no live collection."""
...
TICKET = "CURRENT-TEAM-STRENGTH-001P-L3"
NAMES = (
    "PLANS.md",
    "config/rights/odds_profiles.json",
    "scripts/build_team_strength_l3_review_pack.py",
...
def build() -> dict[str, object]:
    if len(NAMES) != len(set(NAMES)) or len(NAMES) > 20:
        raise ValueError("L3 review entry cap exceeded")
    contents: dict[str, bytes] = {}
    for name in sorted(NAMES):
        path = ROOT / name
        if path.is_symlink() or not path.is_file() or not path.resolve().is_relative_to(ROOT):
            raise ValueError("unsafe L3 review path")
        body = path.read_bytes()
        if len(body) > 1024 * 1024:
            raise ValueError("L3 review entry too large")
        contents[name] = body
    if sum(map(len, contents.values())) > 5 * 1024 * 1024:
        raise ValueError("L3 review payload too large")
    manifest = {
        "ticket": TICKET,
    ...
    if output.is_symlink() or not output.resolve().is_relative_to(ROOT):
        raise ValueError("unsafe L3 review destination")
    ...
    with zipfile.ZipFile(output) as archive:
        if len(archive.namelist()) != len(contents) + 1:
            raise ValueError("L3 review entry count differs")
        for name, body in contents.items():
            if archive.read(name) != body:
                raise ValueError("L3 review verification failed")
    return {
        "status": "PASS",
