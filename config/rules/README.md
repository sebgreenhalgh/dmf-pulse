...
Compiled outputs are explicit operator artifacts under `artifacts/rules/` and are never resolved through a mutable `latest` alias. An approved VERIFIED ruleset can be activated only into an explicit immutable registry path. No target-season rule is inferred from this directory.
