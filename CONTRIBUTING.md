...
4. Do not add a dependency, migration, service, domain stub, provider call, or `.env` design without explicit authority.
...
Use conventional, focused commit messages when the owner requests commits. Codex must not push, merge, tag, rewrite history, or change repository visibility for FND-001.
