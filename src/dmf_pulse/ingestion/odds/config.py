...
class OddsProviderConfig(_FrozenConfig):
    ...
    def validate_allowlists(self) -> OddsProviderConfig:
        ...
        # The Odds API documents request cost as the number of requested
        # markets times regions.  This private EPL boundary fixes one region,
        # so the declared cost must track the two requested markets exactly.
        if self.request_cost != len(self.markets) * len(self.regions):
            raise ValueError("request cost does not match requested market-region coverage")
        return self
